...
const currency = (n: number) => new Intl.NumberFormat("es-CO",{style:"currency",currency:"COP",maximumFractionDigits:0}).format(n||0);
...
export default function POS() {
  ...
  useEffect(() => { setApplyTax(true); }, [settings]);

  useEffect(() => {
    (async () => {
      setLoading(true);
      try { setAll(await fetchProducts()); }
  ...
  function addToCart(p: Product, qty = 1) {
    setCart(prev => {
      ...
      if (ix >= 0) { const copy=[...prev]; copy[ix]={...copy[ix], qty:copy[ix].qty+qty}; return copy; }
      return [...prev, { product: p, qty }];
    });
    setQ("");
  }
  const inc = (i:number) => setCart(prev => prev.map((it,idx)=> idx===i?{...it,qty:it.qty+1}:it));
  const dec = (i:number) => setCart(prev => prev.map((it,idx)=> idx===i?{...it,qty:Math.max(0,it.qty-1)}:it).filter(it=>it.qty>0));
  const remove = (i:number) => setCart(prev => prev.filter((_,idx)=>idx!==i));
  function clearCart(){ setCart([]); setDiscountAbs(0); setDiscountPct(0); setCash(0); }

  const subTotal = useMemo(()=> cart.reduce((s,it)=> s + Number(it.product.price||0)*it.qty, 0), [cart]);
  ...
  async function reallyPlaceSale() {
    ...
    if (discountPct > maxPct && (settings?.pos.require_discount_pin ?? false)) {
      toast.error(`Descuento % supera el máximo permitido (${maxPct}%).`);
      return;
    }
    const falta = cart.find(it => (it.product.stock ?? 0) < it.qty);
    if (falta) { toast.warning(`Stock insuficiente: ${falta.product.name}`); return; }

    setPlacing(true);
    try {
      const res = await createSale({
        items: cart.map(it => ({ product_id: it.product.id, qty: it.qty })),
        discount_pct: discountAbs>0 ? undefined : discountPct || undefined,
        discount_abs: discountAbs || undefined,
        apply_tax: applyTax,
        cash,
      });
      toast.success(`Venta #${res.sale_id} registrada. Total ${currency(res.total)}`);
      clearCart();
    ...
    <div className="flex-1 p-6 space-y-6 animate-premium-fade">
          ...
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {loading ? (
              ...
              <div key={p.id} className="flex items-center justify-between rounded border p-2">
                ...
                <Button size="sm" onClick={()=>addToCart(p)}><Plus className="h-4 w-4 mr-1"/>Agregar</Button>
              </div>
            ))}
          </div>
        </CardContent>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="glass-card lg:col-span-2">
          <CardHeader><CardTitle>Carrito</CardTitle><CardDescription>Productos seleccionados</CardDescription></CardHeader>
          <CardContent>
                    ...
                    return (
                      <TableRow key={it.product.id}>
                        ...
                        <TableCell className="text-right">
                          <Button size="icon" variant="destructive" onClick={()=>remove(idx)}><Trash2 className="h-4 w-4"/></Button>
                        </TableCell>
                      </TableRow>
                    );
                  })}
            ...
            {cart.length>0 && <div className="mt-4 flex justify-between"><Button variant="outline" onClick={clearCart}>Vaciar carrito</Button></div>}
          </CardContent>
        </Card>

        <Card className="glass-card">
          <CardHeader><CardTitle>Totales</CardTitle><CardDescription>Descuentos y cobro</CardDescription></CardHeader>
          <CardContent className="space-y-4">
            <div className="flex justify-between text-sm"><span>Subtotal</span><span className="font-mono">{currency(subTotal)}</span></div>

            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-muted-foreground">Descuento %</label>
                <Input type="number" value={discountPct||""} onChange={(e)=>setDiscountPct(Math.max(0, Number(e.target.value||0)))} placeholder="0"/>
              </div>
              <div>
                <label className="text-xs text-muted-foreground">Descuento $</label>
                <Input type="number" value={discountAbs||""} onChange={(e)=>setDiscountAbs(Math.max(0, Number(e.target.value||0)))} placeholder="0"/>
              </div>
            </div>

            <div className="flex items-center gap-2">
              <input id="tax" type="checkbox" checked={applyTax} onChange={e=>setApplyTax(e.target.checked)} />
              <label htmlFor="tax" className="text-sm">Aplicar IVA ({settings?.taxes.vat_percent ?? 0}%)</label>
            </div>

            <div className="flex justify-between text-sm"><span>IVA</span><span className="font-mono">{currency(tax)}</span></div>
            <div className="flex justify-between text-base font-semibold"><span>Total</span><span className="font-mono">{currency(total)}</span></div>

            <div>
              <label className="text-xs text-muted-foreground">Efectivo</label>
              <Input type="number" value={cash||""} onChange={(e)=>setCash(Math.max(0, Number(e.target.value||0)))} placeholder="0"/>
            </div>
            <div className="flex justify-between text-sm"><span>Cambio</span><span className="font-mono">{currency(change)}</span></div>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Se venderán {cart.reduce((s,i)=>s+i.qty,0)} unidades. Total a cobrar: <b>{currency(total)}</b>.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                  <AlertDialogCancel>Cancelar</AlertDialogCancel>
                  <AlertDialogAction onClick={reallyPlaceSale}>Confirmar</AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
            </AlertDialog>

            <p className="text-xs text-muted-foreground">*El stock se descuenta automáticamente y la venta queda registrada.</p>
          </CardContent>
        </Card>
      </div>
    </div>
